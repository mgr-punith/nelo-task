...
export default function DashboardPage() {
  const { user, loading, logout, checkProtected, isAuthenticated, token } = useJWTSession()
  ...
  async function fetchTasks() {
    ...
    const url = `/api/tasks?filter=${filter}&search=${encodeURIComponent(debouncedSearch)}`
    const res = await fetch(url, {
      headers: token ? { Authorization: `Bearer ${token}` } : {}, // Send JWT token
    })
